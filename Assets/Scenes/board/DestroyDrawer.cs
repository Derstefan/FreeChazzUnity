...
{
  public static void startDestroyAnimation(GameObject gameObject){
        int sub = gameObject.transform.childCount;
        for (int i = 0; i < sub; i++)
        {
            SplitMesh(gameObject.transform, gameObject.transform.GetChild(i));
        }
    }

public static void SplitMesh(Transform parent, Transform transform)
{
    ...
    for (int i = 0; i < partCount; i++)
    {
        ...
        newRigidbody2D.gravityScale = 0.8f;
        ...
        newRigidbody2D.AddForceAtPosition(center.normalized * 2.4f, center, ForceMode2D.Impulse);
        ...
        meshFadeOut.Initialize(newMeshRenderer.material,3.0f);
    }
